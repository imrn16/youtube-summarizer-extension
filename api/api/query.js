...
export default async function handler(request, response) {
	...
	try {
		...
		const openrouterRes = await fetchWithRetries("https://openrouter.ai/api/v1/chat/completions", {
			...
			body: JSON.stringify({
				...
				messages: [
					{
						role: "system",
						content: "You answer questions about a YouTube video with structured, concise sections. Use **bold** sparingly and include timestamps if provided (in square brackets).",
					},
		...
		return response.status(200).json({ answer, correlationId, durationMs: Date.now() - startMs });
	} catch (e) {
